...
void initialize_h5_file(const struct arguments *args) {
    ...
    strncpy(h5_manager.filename, filename, sizeof(h5_manager.filename));
    // Create file
...
void run_acquisition(katherine_device_t *device, const katherine_config_t *config, const struct arguments *args) {
    ...
    snprintf(command, sizeof(command),
            "python3 /home/adisha/git/libkatherine/analysis/analysis_forPresentation.py %s",
            h5_manager.filename);

    int sys_res = system(command);

    if (sys_res != 0) {
        printf("Warning: Visualization script failed to run.\n");
    }
}