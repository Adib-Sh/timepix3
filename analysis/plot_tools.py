...
def fit_skew_normal(data, bins=18, data_type="All Pixels", legend_loc='upper left', output_dir=""):
    ...
    peak_tot_val, peak_height_hist = peak_tot(data, bins=bins) 

    # PDF
    x = np.linspace(min(data), max(data), 10000)
    pdf = skewnorm.pdf(x, shape, loc, scale)

    mean, var = skewnorm.stats(shape, loc, scale, moments='mv')
    median = skewnorm.median(shape, loc, scale)
    error = np.sqrt(var)

    # Negative PDF for left skew dist
    def neg_pdf(x_val):
        return -skewnorm.pdf(x_val, shape, loc, scale)
    
    lower = np.min(data)
    upper = np.max(data)
    mode_result = minimize_scalar(neg_pdf, bounds=(lower, upper), method='bounded')
    mode = mode_result.x
    ...
    # Calculate FWHM and midpoint
    fwhm = right - left
    middle_fwhm = (left + right) / 2
    ...
    pdf_scaled = pdf * len(data) * bin_width  # Scale PDF to match counts
    
    ax.plot(x, pdf_scaled,
            color='deepskyblue', linewidth=2,
            label=(f'Skew-Normal Fit\n'
                   f'Mode={mode:.4f}, Median={median:.4f}, Mean={mean:.4f}\n'
                   f'Error = {error:.4f}'))
    
    # FWHM vertical line
    ax.axvline(middle_fwhm, color='crimson', linestyle='--', linewidth=2,
               label=f'FWHM = {fwhm:.4f}\nMidpoint = {middle_fwhm:.4f}')
    ...
    # Peak of histogram
    ax.plot(peak_tot_val, peak_height_hist, 'ro', label=f'Peak ToT (hist) = {peak_tot_val:.4f}')
    ...
    plt.savefig(os.path.join(output_dir, f'ToT Distribution {data_type}'), dpi=150, bbox_inches='tight')
    plt.show()
...
# ======================    
    stats_dict = {
        ...
        'mean': mean,
        'fwhm': fwhm,
        'fwhm_midpoint': middle_fwhm,
        'peak_tot(fit)': x_peak,
        'peak_tot(histogram)': peak_tot_val,
        'error': error,
        'shape': shape,
        'loc': loc,
        'scale': scale
...
def write_csv(filename, df):
    ...
    if not file_exists:
        print(f"Created new CSV and wrote data: {filename}")
    else:
        print(f"Appended data to existing CSV: {filename}")
