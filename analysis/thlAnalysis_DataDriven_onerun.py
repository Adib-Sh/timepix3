...
energy_file_map = {
    "16keV": "thlscan_datadriven_20250608_104734.h5",
    "14keV": "thlscan_datadriven_20250608_113454.h5",
    "12keV": "thlscan_datadriven_20250608_115511.h5",
    "10keV": "thlscan_datadriven_20250608_121123.h5",
    "8keV":  "thlscan_datadriven_20250608_122537.h5"
}
...
for energy_keV, fname in energy_file_map.items():
    ...
    print(f"Analyzing now: {energy_keV}")
    ...
    plt.title(f'Threshold vs Hit Count for Active Pixels at {energy_keV}', fontsize=14)
    ...
    plt.title(f'Number of Active Pixels vs Threshold at {energy_keV}', fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    plt.title(f'Total Hits vs Threshold at {energy_keV}', fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.yscale('log')
    plt.tight_layout()
    ...
    print(f"[INFO] Finished analysis for {energy_keV}")
...
palette = sns.color_palette('tab10', len(energies))
energy_colors = {e: palette[i] for i, e in enumerate(energies)}
...
for fit in fit_results:
    ...
    # Plot raw data
    plt.scatter(x_data, y_data, s=20, alpha=0.5, color=color, label=f"{energy} data")

    # Plot fit
    plt.plot(x_fit, y_fit, linewidth=2, color=color, label=f"{energy} fit (x₀ = {x0:.3f} ± {x0_err:.3f} mV)")

    # Get y-value at x₀ for better placement
    y0_pos = np.interp(x0, x_fit, y_fit)

    # Plot x0 marker
    plt.errorbar(x0, y0_pos, xerr=x0_err, color=color, fmt='o', capsize=5, label=f"x₀ at {energy}")

plt.xlabel('Threshold (mV)', fontsize=12)
plt.ylabel('Total Hits (log scale)', fontsize=12)
plt.title('Sigmoid Fits and x₀ Across Energies', fontsize=14)
plt.yscale('log')
plt.gca().invert_xaxis()
plt.xlim(x_fit.max(),550)
plt.grid(True, alpha=0.3)
plt.legend(fontsize=8, loc='best', ncol=2)
plt.tight_layout()
plt.savefig("combined_fits_with_x0_clean.png", dpi=300)
plt.show()


# Plot X0 versus Energy

def linear(x, a, b):
    return a * x + b


energies_keV = np.array([float(fit['energy'].replace('keV', '')) for fit in fit_results])
x0_values = np.array([fit['x0'] for fit in fit_results])
x0_errors = np.array([fit['x0_err'] for fit in fit_results])
popt, pcov = curve_fit(linear, energies_keV, x0_values, sigma=x0_errors, absolute_sigma=True)
a, b = popt
...
plt.ylabel('x0 (mV)')
plt.title('x0 vs Energy with Linear Fit')
plt.grid(True, alpha=0.3)
plt.legend()
plt.tight_layout()
plt.savefig('x0_vs_energy_linear_fit.png', dpi=300)
plt.show()